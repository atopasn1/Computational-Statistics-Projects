...
Top_Reviews
```
Solomon Islands, Cuba and Congo are the top countries when you remove the countries with less than 10 reviews.
...
```
On average, which category of chocolate percentage is most highly rated? Do these countries mostly agree or are there disagreements? -> It seems that the two middle categories of chocolate are the most highly rated. I would say that most countries agree however there is missing data from several countries, such that they only have two categories of chocolate percentage. The addition of more data may change this.
...
NA_Values %>%  
  group_by(country) %>%  
  summarize(n=n()) #11 Counties that need to be changed
...
#note to self: you have to add true at the end because if not it will mutate everything else that is not part of the above command to be NA
...
```
-For this plot I chose to plot ingredients and ratings in a violin plot. I chose to use the original ingredients column as the X and now the binary version that we created, making it hard to read and interpret. For a more useful graph I should have done something like the one we made for part 4 where the individual features are faceted.
-I made the X axis incredibly tiny. This makes it virtually unreadable so you don't know what you're looking at and forces the reader to find this information elsewhere.
...
-I made the Y-axis incredibly large such that it looks strange and makes the graph hard to read. This forces the reader to look at this graph for much longer.
-I flipped the Y-axis text upside down so that it becomes even more difficult to read.In a good graph it may be useful to have the text at a angle but fully upside down doesn't work.
-I made the Y-axis a strange pink color that is different from the colors of the rest of the text of the graph. In a better graph I would try and keep the color of all of the text more consistent.
-The title of the graph is so big it wraps around the graph and compresses it making it hard to read such a small graph.In a better graph it would be big but not so big it is a problem.
-The title is upside down and a different color than the rest of the graph so that you have no idea what your'e looking at when you look at an already badly made and tiny graph.
-The actual color of the graph is a mix of 3 colors that are not color-blind friendly. For a future better graph I would want to use one of the pre-set color palettes in R that are color blind friendly.
-The x and Y axis are not labled in a helpful manner and neither is the title so that even if things were not upside down you would not be able to understand what you are looking at. A future better graph would have a more helpful set of axis.
...
3.I got rid of the fill and the key because it was redundant data and it did not show the years well. The X axis already has that information.
4. I added a blue color to the plot so it is more distinct and visible, especially from far away. This also helps it stand out from the grey background.
